...
// Apify token lookup/decryption now happens only in the internal
// credential-fetch endpoint the Actions runner calls itself
// (GET /api/internal/apify-credential -- see routes/internal.js), not
// here. This file no longer needs to touch the token at all.

// NOTE: getOwnerApifyToken() / the decrypted token are intentionally NOT
// sent in the GitHub repository_dispatch payload anymore -- that payload
// is visible in GitHub's event/workflow metadata and Actions logs. The
// workflow instead calls GET /api/internal/apify-credential?job_id=...
// (see routes/internal.js) using the shared INTERNAL_SCRAPE_SECRET to
// fetch the token itself, scoped to this specific job.
